...
```python
from langchain.chat_models import BaseChatModel                         # :3

def create_chat_model(name=None, thinking_enabled=False, *,              # :82
                      app_config=None, attach_tracing=True, **kwargs) -> BaseChatModel:
    ...
    model_class = resolve_class(model_config.use, BaseChatModel)        # :110  反射加载 + 类型校验
    ...
```
...
```python
from langchain.tools import InjectedToolCallId, tool   # :6
from langchain_core.messages import ToolMessage        # :7
```
...
```python
from langchain.tools import InjectedToolCallId, tool   # :9  委派子智能体的工具
```
...
```python
def _get_runnable_config(self, thread_id: str, **overrides) -> RunnableConfig:   # :207
    ...
    return RunnableConfig(
        ...
        recursion_limit=overrides.get("recursion_limit", 100),   # :218  默认上限 100 步
        ...
    )
```
...
```python
from langchain_core.callbacks import BaseCallbackHandler   # :13
```
...
```python
from langchain_core.callbacks import BaseCallbackHandler   # :28
```
