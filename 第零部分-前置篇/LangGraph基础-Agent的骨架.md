...
```python
from langchain.agents import create_agent   # :31
```
...
```python
from langchain.agents import AgentState          # :3
...
class ThreadState(AgentState):                   # :111
...
| 钩子 | 时机 | 典型用途 |
|------|------|---------|
| `before_agent` | 整次运行开始前 | 初始化沙箱、加载上传 |
| `after_agent` | 整次运行结束后 | 清理、记录最终结果 |
| `before_model` | 每次调模型前 | 注入动态上下文(日期/记忆) |
| `after_model` | 每次调模型后 | 摘要、token 预算、循环检测 |
| `modify_model_request` | 改模型请求 payload | 合并 SystemMessage |
| `wrap_model_call` / `wrap_tool_call` | 包裹模型/工具调用 | 审计、错误兜底、注入消息 |
...
```python
from langchain.agents import AgentState                                       # :28
from langchain.agents.middleware import AgentMiddleware                        # :29
from langchain.agents.middleware.types import ModelCallResult, ModelRequest, ModelResponse   # :30
from langchain_core.messages import AIMessage, HumanMessage                    # :31
from langgraph.runtime import Runtime                                          # :32
...
class TokenBudgetMiddleware(AgentMiddleware[AgentState]):                      # :65
...
```python
if config.type == "memory":                       # :59
    from langgraph.checkpoint.memory import InMemorySaver
    yield InMemorySaver()
    return
if config.type == "sqlite":                       # :66
    from langgraph.checkpoint.sqlite import SqliteSaver
    with SqliteSaver.from_conn_string(...) as saver: ...
if config.type == "postgres":                     # :80
...
```python
from langgraph.types import Checkpointer   # :27  类型注解

def get_checkpointer() -> Checkpointer:    # :107  全局单例工厂
    ...
```
...
```python
for item in self._agent.stream(           # :681  (同步路径;异步用 astream,见 :554)
    ...,
    stream_mode=["values", "messages", "custom"],   # :685
    ...
):
    ...
```
...
```python
# * ``run_agent`` is ``async def`` and uses ``agent.astream()``;   # :554
```
...
```python
from langgraph.graph import END                          # :12  终止运行
from langgraph.prebuilt.tool_node import ToolCallRequest # :13
from langgraph.types import Command                      # :14  改状态+跳转
```
...
```python
from langgraph.config import get_config     # :6
from langgraph.types import Command         # :7
```
...
```python
from langgraph.config import get_config    # :8
from langgraph.runtime import Runtime      # :9
```
...
```python
from langgraph.config import get_stream_writer   # :11  子智能体进度事件
```
...
```python
from langgraph.constants import TAG_NOSTREAM           # :14
from langgraph.graph.message import REMOVE_ALL_MESSAGES   # :15
from langgraph.runtime import Runtime                   # :16
```
