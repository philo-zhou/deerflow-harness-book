...
> 前四章给了零件(P1 LangChain 原语)、骨架(P2 LangGraph 图与中间件)、能力注入与四模式(P3)、五条竖切的管线(P4)。这一章把镜头拉到最远,只盯一件事:**用户在浏览器敲下回车,到屏幕上吐出第一个字,这一条线从头到尾是怎么走完的**。
>
...
> 读法建议:**第一次读这本书,先读前言和这一章**,在脑子里搭起这张整图;然后每读一章竖切的深度,就回到这张图上标个"我已经看懂这一站了"。读完 18 章,这张图上每一站都亮了,整条管线就彻底通了。
...
*这一章是前置篇的收尾。读完它,你已经拥有了一张可以来回走的整图。正篇的 18 章会带你逐站下车,钻进每一站的实现细节——但无论钻多深,记得随时回到这张图,知道自己在哪一站。*
